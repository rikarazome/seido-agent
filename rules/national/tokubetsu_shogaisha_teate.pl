...
% 30,450 JPY/month (R8 Apr, verified from official sources). Age >= 20, severe
% disability (v1: shintai_1 only as proxy for detailed assessment).
% Income limit: 3,661,000 + 380,000 * dependents (R8, Katsushika official).
...
tokusho_limit(N, L) :- integer(N), N >= 0, L is 3661000 + 380000 * N.
...
kettei_status(P, self, decided(monthly(30450))) :-
    claimant(P), val(shogai_techo(P), shintai_1),
    val(income(P), V), val(fuyou_ninzu(P), N),
