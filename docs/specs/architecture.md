...
| レート制限 | アプリ層トークンバケット: IPあたり /api/chat 毎分5回、/api/judge 毎分20回。IPはCloud Runが付与する `X-Forwarded-For` 末尾のクライアント値を使用。**インメモリ実装のためインスタンスごとに独立**（max=2なので実効上限は2倍。デモ規模では許容と明記） |
