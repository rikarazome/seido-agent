...
| メインAPI | POST /api/judge | **POST /api/chat**（実装済み） | 対話型UIの中核エンドポイント |
...
| 入力方式 | **対話型チャットUI**（実装済み: 構造化オンボーディング＋チップ＋自由入力のハイブリッド）。Geminiが自由入力から事実抽出 | 自然な対話で複数事実を同時取得。選択肢チップもチャット内に埋め込み |
...
ブラウザ（チャットUI）
 │
 ├─ POST /api/chat    … 対話1ターン（Gemini + Prolog判定）【実装済み】
...
[Step 3] Prolog判定: 全制度 × 全対象に kettei_status/3 を照会
    ...
    │
    ▼
[Step 3b] Prolog判定（差分基準）: マージで事実が増えた場合のみ、
    │  マージ前factsでも判定し、件数変化（decided N件→M件、月額下限）を機械計算
    │
    ▼
[Step 4] Gemini: 応答生成（役割は「理解の確認」+ 一般用語の説明に限定）
    │  入力: [TURN DATA] = 反映された事実の差分 + 件数変化（機械計算値）+ 次の質問
    │  出力: ①発話から何を理解したかの確認 ②カードへの誘導（件数変化のみ）③次の質問
    │        ④用語質問（「所得って何？」等）への一般説明（免責付き）
    │  制約: **制度名・個別金額を書かない**（判定内容の表示はカードのみ =
    │        金額がLLMを通る経路を排除）。特定の制度の内容をLLM知識から説明しない
    │        （条文リンク・窓口へ誘導）。断定表現禁止。TURN DATAにない数値禁止。
    │        反映されなかった事実を反映されたかのように言わない
    │
    ▼
チャット応答（判定カード = 判定表示の唯一のチャネル + 理解確認の短文）
```
...
### Step 4: 応答生成のシステムプロンプト要点（実装: chat.py `RESPONSE_SYSTEM_PROMPT`）

返信の役割は**理解の確認**に限定する。判定内容（制度名・金額）の表示はカードだけが行う。
これにより「お金の数字がLLMの口を通る」経路がプロダクトから消える（Prolog由来保証の徹底）。

```
役割（この4つだけ）:
1. 発話から何を理解したかを短く確認（[TURN DATA]の「反映された情報」のみ。
   マージで破棄された内容を反映されたかのように言わない）
2. 判定カードの更新を件数変化で案内（例:「受給見込みがN件からM件に増えました。
   詳細は画面上部の判定カードでご確認いただけます」）
3. [TURN DATA]に次の質問があれば自然な流れで尋ねる
4. 用語や質問の意図を聞かれたら返信の冒頭でまず答える。一般用語（所得と年収の
   違い等）は一般説明+「正確な定義は窓口へ」の免責付き。**特定の制度の内容・
   金額・条件はLLM知識から説明しない**（判定カードの条文リンクと窓口へ誘導）

禁止事項:
- 制度名を書かない・列挙しない・個別金額を書かない
- 特定の制度の内容・条件をLLM知識から説明しない
- 該当/非該当を判断・示唆しない
- [TURN DATA]にない数値を書かない
- 断定表現（「もらえます」「対象です」）禁止

その他: 初回応答は法的助言でない旨を添える／読み取れず質問でもなければ正直に
伝え選択肢回答か言い直しを促す／センシティブ話題への配慮／2〜4文で簡潔に

TURN DATAヘッダーの限定: 「このデータのみに基づく」制約は件数・金額・判定に
関する記述に限定（役割4の一般用語説明と衝突させない。衝突があると用語質問への
回答可否がサンプリング運になる — 2026-07-12に実測で確認済み）
```

[TURN DATA]（Step 4への入力）は全て機械計算値: 反映された事実の差分（`_applied_diff` —
マージを生き残ったものだけ）、decided件数変化・月額下限変化（マージ前後の判定を比較）、
次の質問（質問選定アルゴリズムの出力）。制度名・per-program金額は**意図的に含めない**。
...
| 対話からの事実抽出 | ランタイム（/api/chat Step 1）【実装済み】 | Gemini | Flash系 | ~0.5円/ターン |
| 理解確認の応答生成（制度名・金額は書かない） | ランタイム（/api/chat Step 4）【実装済み】 | Gemini | Flash系 | ~0.5円/ターン |
...
| エンドポイント | 役割 | 実装状況 |
|---|---|---|
| POST /api/chat | メイン（対話1ターン、Gemini+Prolog） | 実装済み |
...
3. 全制度判定（/api/judge と同じロジック）。マージで事実が増えた場合はマージ前でも判定し件数変化を機械計算
4. Gemini で応答生成（入力は [TURN DATA] = 反映差分+件数変化+次の質問のみ。制度名・金額は渡さない）
5. レスポンス返却
...
- **画面に見えるものは履歴にも入る**（2026-07-12、実バグからの教訓）: チップ回答と
  機械質問バブルは /api/judge 直行で Gemini を通らないが、履歴に入れないと
  後続の自由入力（「わからないです」等）が**1つ前の質問**に対する返答として誤解釈される。
  実装: 機械質問は表示時に model 発話として push、チップ/数値回答時は
  質問+回答を**1つの圧縮エントリ**（`（「<質問>」に選択肢で回答: <ラベル>）`）に統合
  （履歴末尾がその質問のときのみ pop して統合 — 間に挟まった自由会話は消さない）。
  圧縮によりチップQ&Aは8枠中1枠しか使わず、自由会話の押し出しとトークン増を防ぐ
...
## フロントエンド設計（チャットUI）【実装済み — 2026-07-08】

> **実装形態**: 完全自由対話ではなく**ハイブリッド**。生年月日・子ども・年収は構造化
> オンボーディング（チップ/セレクタ）で確実に収集し、初回判定後に自由入力欄を有効化。
> 自由入力は /api/chat（Gemini抽出→Prolog判定→Gemini応答）、チップ回答は従来どおり
> /api/judge 直行（0円）。発話で区が判明した場合はセレクタを切り替えて /api/judge で
> 再判定。503でチャットのみ恒久無効化（チップは無傷）。history はクライアント側で
> 8件×400字にキャップ（サーバの10KBリクエストガード対策）。

### 画面構成（2026-07-12改訂: 判定カードをチャット外の固定ドックへ）

```
┌─────────────────────────────────────────────┐
│  制度エージェント                 [区: 渋谷区 ▼] │
├─────────────────────────────────────────────┤
│ ┌─ 判定サマリーバー（常時固定・タップで開閉）─┐ │
│ │ 受け取れる見込み 月15,000円 一時金20,000円  │ │
│ │ 受給見込み5件・あと少し77件  22/99判定済み ▼│ │
│ └───────────────────────────────────────────┘ │
│   （開くと制度一覧パネルがチャットの上に        │
│     オーバーレイ表示: 金額・詳細・条文・証明木） │
├─────────────────────────────────────────────┤
│                                             │
│  [user] 去年離婚してひとり親です。            │
│                                             │
│  [agent] ひとり親でいらっしゃることを確認      │
│  しました。受給見込みが5件から9件に増えました。 │
│  詳細は画面上部の判定カードでご確認ください。   │
│  税法上の扶養親族は何人ですか？                │
│                                             │
│  [0人] [1人] [2人] [3人] [4人] [5人以上]      │
│                                             │
...
1. **チャットは会話専用、カードは固定ドック**: 判定カードはチャットの流れに挿入
   **しない**（挿入すると巨大なカードが返信を画面外に押し流す — 2026-07-12に
   実ユーザー指摘で改訂）。ヘッダー直下のサマリーバーが常時表示され、タップで
   詳細パネルがオーバーレイで開閉。判定変化時はバーがフラッシュして更新を通知
2. **選択肢チップ**: blocked制度の追加質問は選択肢チップとして提示（タップで即回答）
3. **カードはリアルタイム更新**: 新しい事実が追加されるたびにドックが更新される
