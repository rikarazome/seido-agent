...
ASKABLE_MAP = {
    ...
    "ikuji_kyuugyou": ("ikuji_kyuugyou", "claimant"),
    "sai_shushoku_teate_jukyuu": ("sai_shushoku_teate_jukyuu", "claimant"),
    "tokutei_shishutsu_ari": ("tokutei_shishutsu_ari", "claimant"),
}
...
def facts_to_prolog(facts: dict, as_of: date) -> str:
    ...
    lines = [
        ...
        # claimant and per-child blocks interleave these predicates;
        # without this, swipl warns ~30 lines per load (log noise, and a
        # hard failure if a future swipl escalates warnings to errors)
        ":- discontiguous claimant/1, child/1, kango_by/2, seikei_futan/2, "
        "age/2, age_nendo_matsu/2, birth_nendo/2.",
        f"claimant({CLAIMANT}).",
    ]
    fy = fy_end(as_of)
